(function() {
    ...
    function getAttributeIcon(attribute) {
        ...
        const found = attrList.find(item => item.value === attribute);
        return found ? found.left : '';
    }

    function getAttributeDebuffIcon(attribute) {
        const attrList = window.AppData.abilitytxticon.Attributeicon;
        const found = attrList.find(item => item.value === attribute);
        return found ? found.right : '';
    }
    ...
    /**
 * 将能力效果文本中的特定关键词替换为带图标的 HTML
 * @param {string} text 原始文本
 * @param {string} attribute 当前卡牌属性（火/水/风/雷/光/暗）
 * @param {boolean} isLeader 是否为队长能力（决定是否处理 <main>/<sub> 标记）
...
function enhanceAbilityText(text, attribute, isLeader = false) {
    ...
    // 队长能力特殊处理：将 <main> 和 <sub> 替换为主/副属性图标
    if (isLeader) {
    ...
    function renderAbilityData(container, data, attribute) {
        ...
        // 特殊处理：能力组（title === '能力' 且存在 subsections）
            ...
            data.subsections.forEach(sub => {
                ...
                if (sub.content) {
                    if (typeof sub.content === 'string') {
                        ...
                        lines.push(String(sub.content));
                    }
                }
                if (sub.text) {
                    if (typeof sub.text === 'string') {
                        lines.push(...sub.text.split('\n').filter(l => l.trim()));
                    } else if (Array.isArray(sub.text)) {
                        sub.text.forEach(t => {
                            if (typeof t === 'string') {
                                lines.push(...t.split('\n').filter(l => l.trim()));
                            } else if (t) {
        ...
        if (data.title) {
            ...
            const titleType = data.titleType || data.type;
            if (titleType) {
                titleEl.classList.add(`ability-section-title-${titleType}`);
            }
    ...
    if (buttonValue === 'info') {
        ...
        if (!isMobileMode) {
            ...
            rightPanel.appendChild(modal2Container);
        } else {
            mobileRightContainer.innerHTML = '';
            mobileRightContainer.appendChild(modal2Container);
