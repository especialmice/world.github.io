// js/hero/rightDetail.js
// 右侧详情区域模块
import { dom } from './dom.js';
import { state, setActiveCardIndex } from './state.js';
import { showRightLoading, showDetailAreaLoading, replaceContentAfterImagesLoaded } from './loader.js';
import { createRaceItem, appendTextDivs, enhanceAbilityText, createOddItem } from './utils.js';
...
function createRightButtons(updateDetailAreaCallback) {
    ...
    if (rightButtons && rightButtons.buttons) {
        rightButtons.buttons.forEach(btnData => {
            ...
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                if (updateDetailAreaCallback) updateDetailAreaCallback(btnData.value);
            });
...
function createHeaderArea(card, config, updateDetailAreaCallback) {
    ...
    const rightCol = createRightButtons(updateDetailAreaCallback);
...
export function renderAbilityData(container, data, attribute) {
    if (!data) return;
...
export function renderRightModal2(card) {
    showRightLoading(state.isMobileMode ? dom.mobileRightContainer : dom.rightPanel);

    const config = window.AppData.cardConfigs[state.activeCardIndex];
    if (!config) return;
    ...
    let updateDetailAreaCallback = null;

    const headerArea = createHeaderArea(card, config, (btnValue) => {
        if (updateDetailAreaCallback) updateDetailAreaCallback(btnValue);
    });
    ...
    function updateDetailArea(buttonValue) {
        ...
        if (buttonValue === 'info') {
            ...
            if (infoTitle) {
                fragment.appendChild(createOddItem(infoTitle.title, window.AppData.sarchtitle[0].leftImg, window.AppData.sarchtitle[0].rightImg));
            }
            ...
            if (state.activeCardIndex === 0 || state.activeCardIndex === 1) {
                ...
                if (abilityItem) {
                    ...
                    if (abilityTitle) {
                        fragment.appendChild(createOddItem(abilityTitle.title, window.AppData.sarchtitle[0].leftImg, window.AppData.sarchtitle[0].rightImg));
                    }
    ...
    updateDetailAreaCallback = updateDetailArea;
